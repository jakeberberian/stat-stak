...
clean_rel_data <- function(df, pitcher_name, date) {
...
xyplot(RelHeight ~ RelSide, data = clean_rel_data(duke, pitcher, date), groups = AutoPitchType,
       auto.key = TRUE,
       aspect = "iso",
       xlim = c(-4.5, 4.5),
       ylim = c(0, 7.5),
...
```{r pitch_loc}
clean_loc_data <- function(df, pitcher_name, date){
